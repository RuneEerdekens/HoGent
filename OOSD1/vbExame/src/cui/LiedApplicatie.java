	...
	private void start() {
		...
		for (int lijstNum = 0; lijstNum < aantal; lijstNum++) {
			...
			for (int liedNum = 0; liedNum < 3; liedNum++) {
				...
				System.out.printf("Je koos een %s nummer van %d bpm.%n%n", lied.geefTempo(),
						lied.getBeatsPerMinut());
		...
		System.out.printf("%s is %d seconde!%n",
				aantal > 1 ? "De gemiddelde duur van je afspeellijsten" : "De duur van je afspeellijst",
				som / aantal);
	}
